...
class Chef
  class AwsEc2
    ...
    class << self
      def get_client(credentials, region)
        Aws::EC2::Client.new(region: region, credentials: credentials)
      end

      def get_vpc(name, client)
      ...
      def get_vpc_igw(name, client)
        get_vpc(name, client).internet_gateways.first
      end

      def get_nat_machine(client, name, vpc_id, region = nil)
      ...
      def get_subnet vpc, name, client=nil
        fail "Client needed if VPC name passed" if vpc.instance_of?(String) && !client
        vpc = get_vpc vpc, client if vpc.instance_of? String
        return nil unless vpc
      ...
      def get_free_eip client, allow_create=true
      ...
      def get_security_group vpc, name, client=nil
        fail "Client needed if VPC name passed" if vpc.instance_of?(String) && !client
        vpc = get_vpc vpc, client if vpc.instance_of? String
        return nil unless vpc
      ...
      def get_iam_client(credentials, region)
        Aws::IAM::Client.new region: region, credentials: credentials
      end

      def get_certificate(name, client)
        begin
      ...
      def get_elb_client(credentials, region)
